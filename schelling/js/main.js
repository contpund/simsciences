...
import { SchellingEngine } from './engine.js';
import { Renderer } from './renderer.js';
import { initControls } from './controls.js';
import { initLanguageSwitcher } from './i18n.js';

const canvas = document.getElementById('sim');
const engine = new SchellingEngine(60);
...
// Lightweight tooltip system (mirrors ep03–ep09). Reads [data-tip] set by i18n.
(function tooltips() {
  let el = null;
...
// The readouts move once a round, not once a frame.
let tick = 0;
function tickFrame() {
  engine.step();
  renderer.draw();
  if ((tick++ % 8) === 0) controls.syncStats();
}
