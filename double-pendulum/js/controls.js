...
export function initControls(engine, renderer) {
  ...
  if (btnPause) {
    btnPause.addEventListener('click', () => {
      ...
      btnPause.textContent = engine.paused ? '▶ Play' : '⏸ Pause';
    });
  }
  ...
  if (btnReset) {
    btnReset.addEventListener('click', () => {
      engine.reset();
      // Re-sync sliders with the reset defaults.
  ...
  // Preset shortcuts
  const presets = {
    btnGentle:   { theta1: 30, theta2: 30,  L1: 1.6, L2: 1.4, m1: 1.0, m2: 1.0, damping: 0 },
